...
export class RelationshipTaxonomy {
  ...
  // Picker option groups (canonical key → option label). The "(generic)" hints
  // mark the catch-all types. Order is the order shown in the dropdown.
  static SELECTABLE_GROUPS = [
    {
      label: 'Spouse / Partner',
      keys: [['husband'], ['wife'], ['spouse', 'Spouse (generic)'], ['partner']],
    },
    {
      label: 'Parents',
      keys: [
    ...
    {
      label: 'Siblings',
      keys: [['brother'], ['sister'], ['sibling', 'Sibling (generic)']],
    },
    {
      label: 'Grandparents',
      keys: [['grandmother'], ['grandfather'], ['grandparent', 'Grandparent (generic)']],
    },
    {
      label: 'Grandchildren',
      keys: [['grandson'], ['granddaughter'], ['grandchild', 'Grandchild (generic)']],
    },
    {
      label: 'Extended Family',
      keys: [['uncle'], ['aunt'], ['nephew'], ['niece'], ['cousin']],
    },
    { label: 'Social', keys: [['friend'], ['neighbor']] },
    { label: 'Professional', keys: [['colleague'], ['manager'], ['assistant']] },
  ];
  ...
  /** Grouped <optgroup> HTML for the relationship-type picker, plus a custom option. */
  static optionsHtml(selectedType) {
    let html = '';
    const selectable = [];
    for (const group of this.SELECTABLE_GROUPS) {
      html += `<optgroup label="${group.label}">`;
      for (const [key, optLabel] of group.keys) {
        selectable.push(key);
        const text = optLabel || this.label(key);
        html += `<option value="${key}"${key === selectedType ? ' selected' : ''}>${text}</option>`;
      }
      html += '</optgroup>';
    }
