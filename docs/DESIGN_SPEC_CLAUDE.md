...
**Picker structure (`PICKER_TREE` / `pickerOptions()` / `optionsHtml(selected, includeCustom)`).** The relationship-type picker is a single flat list (no `<optgroup>` headers). Each generic parent type (`spouse`, `parent`, `stepparent`, `child`, `stepchild`, `sibling`, `grandparent`, `grandchild`) is a selectable top-level option; its gendered / more-specific subtypes (e.g. `husband`, `wife`, `partner` under `spouse`) render **indented** beneath it (three non-breaking spaces, `PICKER_INDENT`). Standalone types (`uncle`, `aunt`, `nephew`, `niece`, `cousin`, `friend`, `neighbor`, `colleague`, `manager`, `assistant`) are flat. There is no "(generic)" label suffix — the user selects either the generic parent or any subtype, and the stored value is always the canonical key. `pickerOptions()` returns `[{ value, label, depth }]` and is shared by `optionsHtml()` (native `<select>`) and the searchable combobox (§7.22). `includeCustom=false` omits the `Custom…` escape hatch for pickers that must yield a known type (the bulk retype selectors).
...
`centerOnContact(id)` — zooms/centres on a node **without** changing selection (used by the detail-card "center on this contact" button). Returns `true` if the node exists and was centred, `false` otherwise.

`resetView()` — animated transition to `d3.zoomIdentity` (still available internally).

The header **Focus on Me** button (`#btn-focus-me`, `app.js`) centres on AND selects the `_selfContactId` ("me") contact (`graph.highlightContact` + `_onNodeSelect`). It is **disabled** whenever no "me" contact is chosen or it is not in the current graph; `_syncFocusMeButton()` (called from `_rebuildGraph`) keeps the disabled state in sync. There is no whole-graph fallback.
...
2. **Contact Info** — read-only rows (`_detailRow(icon, value, label)` renders the **label above the value**), including custom fields from Markdown/import metadata as `Custom: <Label>` rows, or editable form
3. **Notes** — inline textarea with autosave (2-second debounce) and hashtag autocomplete
4. **Relationships** — a single **collapsible** master section (see below)
5. **Footer buttons** — context-sensitive (virtual node shows "Create Contact", edit mode shows Save/Cancel)

The header also carries two icon buttons: a "center graph on this contact" (⊙, `#btn-center-contact` → `graph.centerOnContact(selectedNodeId)`) and the close (✕) button.

#### Relationship Display Sections

The **Relationships** section is a collapsible master section (`_makeCollapsible(title, {collapsed, onToggle, badgeText, badgeClass})` in `app-detail.js`) rendered into `#detail-relationships`. Its body holds, in order:

1. **Own relationships** — from `contact.related`; each row reads `[Type] is [Name]` (a static `.rel-connector` "is" between the type chip and the name) and has an edit (✎) button that triggers inline type editing
2. **Referenced in Others' Cards** (back-references) — a nested **collapsible** subsection (count badge) listing the other contacts whose `related` arrays mention this node; each row reads `[Type] of [Name]` (static `.rel-connector` "of"), with the "listed in <name>'s card" explanation moved to the row's `title` tooltip (the old visible "via <first name>" field was removed)
3. **From "<org>" (Inferred)** — a nested subheader + up to 8 inferred colleague edges; the "+ N more colleagues" overflow is a clickable **reveal link** (`.rel-more-link`) that inserts the remaining inferred rows in place and removes itself
4. **Suggested Additions** — its **own** nested collapsible subsection (with a count badge), built by `_renderSuggestions(node, parentBody)` from the suggestion engine output

Collapse state for the master section, the Referenced subsection, and the Suggested Additions subsection is persisted on the app instance (`_relSectionCollapsed`, `_refSectionCollapsed`, `_suggSectionCollapsed`) so it survives re-selection within a session. The chevron rotates to indicate expanded/collapsed; `.rel-collapse.collapsed > .rel-collapse-body { display: none }`.
...
Saving calls `_saveDetailEdits()`, which delegates the form-DOM→model read to `_commitEditFormToModel(contact)` (also used by the Add-Relationship flow), then rebuilds the graph and exits edit mode. `_commitEditFormToModel` reads all form state (including open inline relationship editors), updates the contact object, and calls `_rewriteEditableFields()` — but does **not** exit edit mode or rebuild.

The **Add Relationship** affordance stays available while editing (footer `#btn-add-rel` and the inline action). Because the add-relationship save regenerates the card from the model, opening the modal during an edit first calls `_commitEditFormToModel(editingContact)` so in-progress field edits are not discarded; edit mode persists across the add.
...
- Relationship Type: a **searchable** `<select>` (§7.22) of the indented type list (generic parents + subtypes) + "Custom…" option
- Custom Type: text input (shown when "Custom…" selected)
- Target: dropdown — "Choose Existing Contact" or "Enter New Name"
- Existing Contact: a **searchable** `<select>` of all real contacts
...
Export goes through the format adapters and supports three **scopes** — a single contact (detail panel), the current multi-select (`_selectedForExport`), or all contacts — each available as vCard, Markdown, or TSV. Each scope is presented as a single **dropdown menu** (the shared `attachMenu` popover, §7.26): "Export All ▾" (`#btn-export-all-menu`, header, revealed once contacts load), "Export Selected ▾" (`#btn-export-selected-menu`, sidebar selection bar), and "Export Contact ▾" (`#btn-export-contact-menu`, detail footer). Each menu's items call the same scope/format export methods below. Bulk filenames carry a `_dateStamp()` (`YYYY-MM-DD`).
...
A rule-based find-and-replace tool. Supports multi-condition IF rules with AND/OR logic. Field definitions (`_bulkRuleFieldDefs()`) drive both the IF condition pickers and the THEN action picker; each has `{ key, label, scope, get, set? }`. The field pickers are **searchable** (§7.22).

**Condition types:** field contains/equals/starts-with/ends-with/is-empty/is-not-empty (scopes: contact, email, address), plus **Relationship Type** (scope `related`) with operators *has type* / *does not have type* — matches contacts having (or not having) a relationship of the chosen type. The condition value for a relationship-type field is itself a searchable type picker.

**Action types:**
- `set` — overwrite a field with a new value
- `append` — append to a field
- `clear` — clear a field
- **Relationship retype** (when the action field is *Relationship Type*): the set/append/clear + value controls are replaced by a "from [type] → to [type]" pair (`#bulk-rel-from` / `#bulk-rel-to`, both searchable). On apply, every relationship of the *from* type on each matched contact is retyped to the *to* type by reusing `_applyRelationshipEdit(contact, idx, name, toType)`, **which also updates the reciprocal relationship on the other person's card** (subject to the reciprocal-downgrade guard, §10.1).

When the action type is **Clear Field**, the value input is hidden (clearing removes the value, so no value is needed). The apply button label is dynamic — **"Apply Rule to N Contacts"** where N is the live affected-contact count (0 and disabled when nothing matches).

Shows a live preview of affected contacts and a sample list before applying (the preview counts relationship retypes per matched contact). Requires the user to check a confirmation checkbox before the apply button is enabled. After apply, the `RelationshipBuilder` and graph are rebuilt and the session persisted.

> Note: the relationship-type **THEN** action currently still uses the dedicated "from type → to type" retype controls; reworking it into the plain Set/Clear form is deferred pending a design decision.
...
### 7.25 Searchable Select (`js/searchable-select.js`)

`makeSearchable(select, { placeholder })` progressively enhances a native `<select>` into a **type-to-filter combobox** while keeping the `<select>` as the single source of truth: it is hidden but remains in the DOM as the value holder, so reading `select.value` and listening for its `change` event keep working unchanged. The enhancer wraps the select in a `.searchable-select` div, adds a text `input` (filters options by case-insensitive substring, ignoring the indentation ` `) and a floating `.searchable-list`; choosing an item sets `select.value`, updates the display, and dispatches a synthetic `change`. Keyboard: ↑/↓ move the active item, Enter selects, Escape closes; clicking outside closes. It is **idempotent** (`refresh()` re-syncs the display after the options or value change) and self-cleans its document listener when the wrapper leaves the DOM (so re-rendered rows don't leak). It also tolerates a **detached** `<select>` (one not yet in the DOM): the wrapper is created but not inserted, and the caller appends the wrapper (= the select's new parent). Callers that toggle visibility must coerce the flag to a real boolean — `classList.toggle(cls, undefined)` *flips* rather than sets.

Applied to the large-list pickers: the Add-Relationship contact picker and type picker, the Bulk Normalize condition field pickers / action field picker / relationship-type condition value / relationship retype from-and-to pickers, and the inline suggestion type picker. (Small 2–3 option selects — operator, action-type, target-mode — are left native.)

### 7.26 Dropdown Menu (`js/menu-button.js`)

`attachMenu(trigger, getItems)` turns a button into a dropdown-menu trigger. `getItems` is an array (or a function returning one, evaluated per open) of `{ label, onSelect, disabled?, danger? }` / `{ separator: true }`. The popover is **fixed-positioned** under the trigger (never clipped by an `overflow:hidden` ancestor), and closes on selection, outside click, Escape, scroll, or resize. Used to consolidate what were many header/footer buttons into:

- **Import ▾** (`#btn-import-menu`) — "Import vCard" (sets `file-input.accept='.vcf,.vcard'` then opens the picker), "Import MD (coming soon)" (placeholder — shows an info toast, no import yet), "Import TSV" (`accept='.tsv'`), separator, "Download TSV Template".
- **Export All / Selected / Contact ▾** (§7.13).
- **Session ▾** (`#btn-session-menu`) — "Restore Last Session" + "Clear Saved Session"; the trigger itself is disabled when `_updateSessionButtons` finds no saved session.

### 7.27 Other UI Conventions

- **Focus on Me** replaces the old Reset View button (§6.8).
- **Graph / Table** is a single **segmented toggle** (`.view-toggle` with `.view-toggle-btn.active`), not two separate buttons.
- **Collapsible panels**: `_initCollapsiblePanels()` (called from `_init`) makes each left-sidebar `.sidebar-section > .sidebar-title` and the graph `.legend-title` click-to-collapse (toggles a `collapsed` class; CSS hides the body and rotates a chevron). In-memory for the session.
- **Empty state**: the `#drop-zone` onboarding panel ("Import to get started" + drag-and-drop + "Choose File…") shows until contacts load; `#file-label` reads "No contacts loaded" when empty.
- **Disabled buttons**: a shared `.btn:disabled` rule (reduced opacity, `pointer-events:none`) grays out unavailable actions (Focus on Me, Session).
...
- **Build stamp** — the upper-left `.build-stamp` (`index.html`) is a static, hand-edited marker with three stacked lines: `Updated`, the date (`YYYY-MM-DD`), and the time (`HH:MM:SS TZ`). It carries no logic; update it on every change so the user can confirm they are viewing the latest build. The stylesheet link also carries a `?v=` cache-buster (`css/styles.css?v=…`) that must be bumped whenever `styles.css` changes, or browsers serve the stale cached CSS.
