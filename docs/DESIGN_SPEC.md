...
| Centering | per-component "home" pull: `forceX`/`forceY` toward the component's packed home, strength 0.06 (no global center force) |
| Component home packing | largest connected component at container center; others packed deterministically on outward rings (component radius estimate `30 + 24√count`, clearance gap 40px, golden-angle start per component) so no two home circles overlap |
...
- **Re-layout** (`graph.relayout()`, header `❖` button): clears `_nodePositions` and pins, re-scatters each node around its component's home (spread `max(60, home.r)`), restarts at alpha 1, and auto-runs `fitView()` after 1.4 s. Component homes come from `ConstellationGraph.computeComponents` (union-find) + `packComponentHomes` (deterministic ring packing), recomputed per render/resize/re-layout; `forceX`/`forceY` accessors are re-set after recomputing because d3 caches accessor results per node at force initialization.
